;(function(){
    ...
    $.ajax({
        ...
        success:function(res){
            ...
            wx.ready(function(){
                ...
                wx.onMenuShareAppMessage({
                    ...
                    link: 'http://h5.365zhuawawa.com/H5/sharePage.html?token='+token+'&userId='+user.id, // 分享链接，该链接域名或路径必须与当前页面对应的公众号JS安全域名一致
                ...
                wx.onMenuShareQQ({
                    ...
                    link: 'http://h5.365zhuawawa.com/H5/sharePage.html?token='+token+'&userId='+user.id, // 分享链接
                    imgUrl: 'http://zww-image-dev.oss-cn-shanghai.aliyuncs.com/238640877592622509.jpg', // 分享图标
                    success: function () {
                    // 用户确认分享后执行的回调函数
                    },
        ...
        $.ajax({
            type:'post',
            url:ip+'/icrane/api/charge/chargeHistory',
            data:{
                "memberId":user.id,
                // "orderStatus":'寄存中',
                "token":token,
            ...
            success:function(data){
                ...
                for(var i = 0; i<res.length;i++){
                    if(res[i].coins > 0){
                        history += '<li><div class="detail"><span>'+res[i].chargeMethod+'</span><span>'+res[i].chargeDate+'</span></div><span class="num green">+'+res[i].coins+'</span></li>'
                    }else{
                        history += '<li><div class="detail"><span>'+res[i].chargeMethod+'</span><span>'+res[i].chargeDate+'</span></div><span class="num red">'+res[i].coins+'</span></li>'
                    }
                }
                $(history).appendTo($('.list'));
            }
        })
    }
